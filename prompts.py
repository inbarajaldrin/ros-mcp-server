...
if there are two or more objects with the same color then pick the closest object (x position)

Step 5: move to lego.

Step 6: move to grasp object

Step 7: close gripper and read the final output, if the output is close to the closing range of gripper then the grasp failed, reset joints and move to step 5 again and continue from there.

Step 8: once the grasp is sucessful, reset joints and move camera reset 

Step 9: move linear -0.4 for 0.2 secs

Step 10: control wheels to move to yaw=0 (location of aruco marker drop container)

Step 11: Read database to see if any ids have been previously assigned to any color. if you have already assigned a id for the object color you have at hand. then add information to database saying you are processing the current lego block of "color" and "id" and update the "count" +1 and set it to processing "status". if the color is not assigned any aruco then based on available aruco ids, assign one id to the color of the lego block you are holding and update database using mqtt.

Step 12: move to aruco

Step 13: move to drop

Step 14: open gripper

Step 15: update database to mark the current task as completed

Step 16: reset joints

Step 17: move linear -0.4 for 0.2 secs

Step 18: control wheels to move to yaw=80 (location of lego blocks)

Step 19: Loop back to step 1 and continue the tasks till there are no lego blocks in the scene.
"""
