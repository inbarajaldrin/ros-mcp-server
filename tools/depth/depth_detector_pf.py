#!/usr/bin/env python3
"""
Combined YOLO + Depth Detection Node
Detects objects with YOLO and annotates them with depth values
"""
...
class YOLODepthDetectorNode(Node):
    def __init__(self):
        ...
        # OpenCV window
        cv2.namedWindow("YOLOE + Depth Detection", cv2.WINDOW_AUTOSIZE)
        ...
        self.annotated_publisher = self.create_publisher(
            Image, 
            '/yolo_depth_annotated', 
            10
        )
        
        self.get_logger().info("🤖 YOLOE + Depth Detector started")
        self.get_logger().info("Subscribing to: /rgb_lego")
        self.get_logger().info("Subscribing to: /depth_lego")
        self.get_logger().info("Publishing to: /yolo_depth_annotated")
    ...
    def synchronized_callback(self, rgb_msg, depth_msg):
        ...
        try:
            ...
            # Run YOLOE prompt-free detection with optimizations
            results = self.model(rgb_image, verbose=False, conf=0.3)
            ...
            # Use YOLO's built-in plotting for base annotations
            annotated_image = results[0].plot(boxes=True, masks=False)
            
            # Add depth information on top of YOLO annotations
            if results[0].boxes is not None and len(results[0].boxes) > 0:
                ...
                for i, (box, cls, conf) in enumerate(zip(boxes, classes, confidences)):
                    ...
                    # Draw depth label below the YOLO annotation
                    if depth_value > 0:
            ...
            # Display image
            cv2.imshow("YOLOE + Depth Detection", annotated_image)
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info("🚀 Starting YOLO + Depth detection...")
        rclpy.spin(node)
    except KeyboardInterrupt:
        print("\n🛑 Shutting down YOLO + Depth Detector Node...")
    except Exception as e:
        print(f"❌ Error: {str(e)}")
    finally:
        if 'node' in locals():
