# Technology Stack — Compliant Insertion Studio

> Moved from `CLAUDE.md` 2026-05-06 to keep the project memory file lean. This is reference material, consulted on demand. **The `Verified Live` section is dated 2026-05-01/03** and describes the state at the close of the WRAP-VERIFY milestone; the package/version facts have held, the project-status lines in it have not. For current state read `docs/HANDOFF_NEXT_AGENT.md`.
...
| **`dashboard_client/recover` for fault recovery** | Clearing a protective stop is a manual action at the pendant. *(Updated 2026-08-16: the pendant is now in **Remote Control** with headless bringup, so `dashboard_client` power-on / brake-release / mode queries do work — but `recover` still is not the way to clear a protective stop.)* | Operator manually clears protective stops on the pendant. Wrapper code must not assume `recover` works. |
...
- **F/T payload calibration recovered** (2026-05-03): mass=2.1109 kg, CoG=[-0.0032, +0.0031, -0.0318]m. **As of 2026-05-06, applied via `/io_and_status_controller/set_payload` ROS service from `launch_robot.sh`**, sourced from the latest `compliant_insertion_studio/configs/ft_calibration_*.yaml`. The earlier "pasted into bringup" claim was wrong — that line never existed in any launch file. Force_mode ran with 0kg gravity comp from project start through 2026-05-05, causing 7-8mm of unexplained TCP drift during APPROACH on every run. **Pendant payload setting does NOT propagate to ROS-side force_mode** — those are independent. Verified 2026-05-06: with `set_payload` active, TCP drift during 19s descent drops 7.85mm → 0.83mm (9.5× tighter); roll Δ -0.42° → -0.03° (12×); yaw Δ +0.79° → +0.21° (3.8×).
- **Reusable orchestrator script validated**: `compliant_insertion_studio/scripts/run_assembly_step.py` works for u_brown grasp_id=1. ~~Other 3 FMB1 grasp_ids need 5-min verification before Phase 3 collection.~~ **Done long since** — all four parts have been in production since 2026-05-07, and `grasp_id` / `gripper_width_mm` auto-resolve from `ablations/eval_resources/fmb1_assembly.json`.
## Sources
