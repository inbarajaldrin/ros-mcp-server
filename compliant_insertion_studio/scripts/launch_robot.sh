...
echo ""
if [[ "$MODE" == "real" && "$HEADLESS" == "true" ]]; then
    echo "  Headless external control — NOTHING TO DO ON THE PENDANT."
    echo "    The driver sent its own control script over the primary interface."
    echo "    Confirm with:"
    echo "      grep 'Ready to receive control commands' $LOG_FILE"
    echo "    If that line never appears, the pendant is probably in Local mode;"
    echo "    headless control cannot be established from Local."
    echo ""
elif [[ "$MODE" == "real" ]]; then
    echo "  NEXT STEPS (real mode — URCap path, pendant in Local):"
    echo "    NOTE: as of 2026-08-16 no program named external_control.urp exists"
    echo "    on this controller. Prefer 'launch_robot.sh real --headless', which"
    echo "    needs no pendant program at all (requires Remote Control mode)."
    echo ""
    echo "    First-time / cold pendant:"
    echo "      1. Power on the robot (touch red panel, follow boot)"
    echo "      2. Release brakes (ON button)"
    echo "      3. Load your External Control program"
    echo "      4. Press Play ▶️"
    echo ""
    echo "    If the pendant was ALREADY running External Control from before"
