#!/usr/bin/env python3
"""
Get Target Grasp Pose

Calculates the target grasp pose in world frame by:
...
5. Loading grasp point data and transforming specific grasp point position 
   from object frame to world frame

Output: JSON containing only the target grasp pose in world frame
"""
...
def get_grasp_point_by_id(grasp_data, grasp_id):
    ...
    for gp in grasp_points:
        # Handle both int and string ID comparisons
        gp_id = gp.get('id')
        if gp_id == grasp_id or str(gp_id) == str(grasp_id):
            return gp
...
def main(args=None):
    parser = argparse.ArgumentParser(description='Get Target Grasp Pose')
    ...
    # STEP 1: Load grasp points data and extract grasp point relative to object FIRST
    # (before any world frame transformations)
    grasp_data = load_grasp_points_data(args.object_name, GRASP_DATA_DIR)
    if grasp_data is None:
        print(f"Error: Could not load grasp points data for {args.object_name}")
        sys.exit(1)

    # Get the specific grasp point by ID (required)
    grasp_point = get_grasp_point_by_id(grasp_data, args.grasp_id)
    if grasp_point is None:
        print(f"Error: Grasp point with ID {args.grasp_id} not found for {args.object_name}")
        sys.exit(1)

    # Extract grasp point position relative to object (local frame)
    grasp_position_relative_to_object = np.array([
        grasp_point['position']['x'],
        grasp_point['position']['y'],
        grasp_point['position']['z']
    ])

    # STEP 2: Transform target object pose from base frame to world frame
    ...
    # STEP 3: Transform grasp point from object frame to world frame
    # Using the target object pose in world frame
    target_grasp_position_abs = transform_grasp_point_to_world(
        grasp_point, target_object_position_abs, target_object_orientation_abs
    )
    ...
    # Output only the target grasp pose as JSON
    result = {
        "target_grasp_pose": {
            "position": {
                "x": round(float(target_grasp_position_abs[0]), 4),
                "y": round(float(target_grasp_position_abs[1]), 4),
                "z": round(float(target_grasp_position_abs[2]), 4)
            },
            "orientation": {
                # Grasp point inherits object orientation
