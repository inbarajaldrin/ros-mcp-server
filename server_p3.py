...
def _set_object_state(object_name: str, orientation: Optional[list] = None, grasp_id: Optional[int] = None) -> None:
    state = _load_state()
...
@mcp.tool()
def clear_primitive_state() -> dict:
    """Clear the primitive state store. Call at phase start."""
    p = _state_path()
    if os.path.exists(p):
        os.remove(p)
