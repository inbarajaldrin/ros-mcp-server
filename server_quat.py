...
# ── Ablation mode remapping ──────────────────────────────────────────────────
# The agent writes mode='real' (which enforces orientation/grasp_id params),
# but all primitives execute in sim. This tests whether the agent accurately
# carries forward numerical values without needing a physical robot.
_ABLATION_REMAP_MODE = True  # Set to False to disable remapping

def _remap_mode(mode: str) -> str:
    """Remap real→sim for ablation. Agent thinks it's real, primitives run sim."""
    if _ABLATION_REMAP_MODE and mode == "real":
        return "sim"
    return mode
...
def verify_grasp(
    object_name: str,
    mode: Mode,
    grasp_id: Annotated[int, Field(description="Grasp point ID from get_scene_info. Required for both sim and real mode.")],
    current_object_orientation: Annotated[List[float], Field(description="Quaternion [x, y, z, w] from move_to_grasp or rotate_object output. Required for both sim and real mode.")],
) -> GraspVerificationResult:
    """Verify if the object is grasped.

    Required: grasp_id and current_object_orientation must be provided for both sim and real mode.
    Pass the quaternion [x,y,z,w] from move_to_grasp or rotate_object output.
...
def translate_object(
    ...
    mode: Mode,
    grasp_id: Annotated[int, Field(description="Grasp point ID used during move_to_grasp. Required for both sim and real mode.")],
    current_object_orientation: Annotated[List[float], Field(description="Quaternion [x,y,z,w] from move_to_grasp or rotate_object output. Required for both sim and real mode.")],
...
Returns:
    ...
    if action != "place_down":
        ...
        if missing:
            return {"result": "failure",
                    "error": f"Required: {', '.join(missing)}. Get from move_to_grasp or rotate_object output."}
...
def rotate_object(
    ...
    base_name: Annotated[str, Field(description="-")],
    mode: Mode,
    current_object_orientation: Annotated[List[float], Field(description="Quaternion [x,y,z,w] from move_to_grasp output. Required for both sim and real mode.")],
) -> RotateObjectResult:
...
- current_object_orientation is required — pass the quaternion [x,y,z,w] from
  move_to_grasp's current_object_orientation output.

Returns:
    result: "success" or "failure"
    initial_object_orientation: {quat: {x, y, z, w}} before rotation
    final_object_orientation: {quat: {x, y, z, w}} after rotation — use this as
        current_object_orientation for subsequent translate_object or verify_grasp calls
    error: failure reason (only on failure)"""
    mode = _remap_mode(mode)
    # Enforce required param — FastMCP does not reject missing required fields at call time
    if current_object_orientation is None:
        return _parse_result(RotateObjectResult, {"result": "failure",
            "error": "current_object_orientation is required. Pass the quaternion [x,y,z,w] from move_to_grasp output."})
    if not isinstance(current_object_orientation, list) or len(current_object_orientation) != 4:
        return _parse_result(RotateObjectResult, {"result": "failure",
            "error": "current_object_orientation must be a list of 4 floats [x, y, z, w]"})
